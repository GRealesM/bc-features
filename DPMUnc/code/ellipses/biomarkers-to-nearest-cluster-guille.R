...
# see https://grealesm.github.io/Bases/cell_basis_v3_varimax/Reports/Cellv3_report_20221111_UKBB.html#Which_datasets_are_significant_overall for details.
# It needs a bit of processing to remove redundant datasets and ensure it matches the DPMUnc files
nr.ds <- fread("../../data/tmp_processed_redundancies_202211_UKBB.tsv") %>% .[ is.na(Remove), Trait] # Redundant were marked with 1, and non-redundant were NA. 
# 120 datasets

pdata <- pdata[ Trait %in% nr.ds]                     # Keep non-redundant traits only. For IMD, the ones that went into DPMUnc
pdata <- pdata[ Trait_class %in% c("IMD", "BMK")]     # Keep IMD and BMK only
pdata <- pdata[ PC %in% paste0("PC", c(2, 5, 8, 11))] # Keep PCs of interest only

# DPMUnc IMD results
pc58 <- readRDS(file.path(DPres.path, "UKBB_PC58_IMD_psm_data.rds"))
pcgr <- readRDS(file.path(DPres.path, "UKBB_PCgrac_IMD_psm_data.rds"))

# Check we're considering the same stuff
IMDtraits  <- unique(pdata[ Trait_class == "IMD", Label])
all(IMDtraits == names(pc58$calls$cl)) # TRUE
all(IMDtraits == names(pcgr$calls$cl)) # TRUE

########################################################################################

## EXPLORATORY DATA ANALYSIS

## Comparing PC58 and PCgrac for cluster assignment
...
ggplot(dtsum, aes(y=y, axis1=cl58,axis2=clgr)) + geom_alluvium(aes(fill=factor(cl58))) +
  ...
  scale_x_discrete(limits = c("PC58", "PCgrac"), expand = c(.05, .05)) +
...
ggsave(file.path(fig.path, "Figure_SXX_alluvial.png"), height = 7, width = 6, bg = "white")
